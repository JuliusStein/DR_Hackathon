...
if st.session_state.summarize:
    feature = st.text_input("Input Feature Name", st.session_state.df.columns[0])
    ...
    feature = st.text_input("Input Feature Name", st.session_state.df.columns[0])
    length = st.number_input("Max Tokens", 10, 100, 30)
    tone = st.selectbox("Tone", ['friendly', 'professional', 'sarcastic', 'formal', 'casual', 'informative'])
...
if st.session_state.df is not None:
    quotes = [str(quote) if quote is not None else "" for quote in list(st.session_state.df[feature])]
